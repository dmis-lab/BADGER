...
def load_dataset_collate_model(conf):
    ...
    if conf.dataprep.baseline_default:
        ...
        DATASET_CACHE = f'{conf.path.dataset}/{conf.model_params.pred_model}_{conf.experiment.dataset}'
        DATASET_CACHE += f'_labeltype_{conf.dataprep.label_type}'
        DATASET_CACHE += f'_splittype_{conf.dataprep.split_type}'
        DATASET_CACHE += f'_usepertid_{conf.dataprep.use_pert_id}'
        DATASET_CACHE += f'_useperttype_{conf.dataprep.use_pert_type}'
        DATASET_CACHE += f'_usecellid_{conf.dataprep.use_cell_id}'
        DATASET_CACHE += f'_usepertidose_{conf.dataprep.use_pert_idose}'
        DATASET_CACHE += f'_filterbytas_{conf.dataprep.filter_tas}'
        DATASET_CACHE += f'_filterbyqc_{conf.dataprep.filter_qc}'
