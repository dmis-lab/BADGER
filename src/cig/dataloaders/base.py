...
def load_cell_embeddings(conf):
    ...
    cell_embedding_table = pd.read_csv(conf.path.dataset+'/cell_embeddings.csv', index_col=0)
...
class CigDatasetBase(Dataset):
    def __init__(self, conf):
        ...
        self.dataset_type = conf.experiment.dataset # only chemprof
        ...
        self.drugcmp_dataframe = pd.read_csv(self.dataset_path+'/drug_smiles.csv', index_col='pert_id')
        if selection_method:
    ...
    # def load_sigfactory(self):
    #     from rdkit import Chem
    #     from rdkit.Chem import ChemicalFeatures
    #     fdefName = '/home/keonwoo/workspace/ChemProf/data/MinimalFeatures.fdef'
    ...
    def make_splits(self):
        ...
        if self.split_type != 'pair_id':
            ...
            for train_pair_ids, test_pair_ids in kf.split(self.unique_meta['pair_id']):
                train_pair_ids, valid_pair_ids = train_test_split(train_pair_ids, test_size=0.25)
                train_pair_ids = np.array(self.unique_meta['pair_id'])[train_pair_ids].tolist()
                valid_pair_ids = np.array(self.unique_meta['pair_id'])[valid_pair_ids].tolist()
                test_pair_ids  = np.array(self.unique_meta['pair_id'])[test_pair_ids].tolist()
                train_pair_ids = [tuple(x) for x in train_pair_ids]
                valid_pair_ids = [tuple(x) for x in valid_pair_ids]
                test_pair_ids  = [tuple(x) for x in test_pair_ids]
                assert len(set(train_pair_ids) & set(valid_pair_ids)) == 0
                assert len(set(valid_pair_ids) & set(test_pair_ids))  == 0
                assert len(set(train_pair_ids) & set(test_pair_ids))  == 0
                
                train_indices = [idx for idx,meta in enumerate(self.meta_instances) if (meta[1],meta[2]) in train_pair_ids]
                valid_indices = [idx for idx,meta in enumerate(self.meta_instances) if (meta[1],meta[2]) in valid_pair_ids]
                test_indices  = [idx for idx,meta in enumerate(self.meta_instances) if (meta[1],meta[2]) in test_pair_ids]   
                assert len(set(train_indices) & set(valid_indices)) == 0
                assert len(set(valid_indices) & set(test_indices)) == 0
                assert len(set(train_indices) & set(test_indices)) == 0
                self.kfold_splits.append((train_indices, valid_indices, test_indices))
    ...
    ######################################################
    #                                                    #
    # This code block will be editted/used in the future #
    #                                                    #
    ######################################################

    # def  hard_samples(self, train_indices, valid_indices, test_indices):
