...
class CigDataset(CigDatasetBase):
    def __init__(self, conf):
        ...
        if conf.dataprep.baseline_default:
            ...
            conf.dataprep.use_pert_idose = True
...
def create_molecule_masks(list_molecules):
    batch_idx = list_molecules['molecules'].get_neighbor_idx_by_batch('atom')
    molecule_length = [len(idx) for idx in batch_idx]
    mask = np.zeros((len(batch_idx), max(molecule_length)))
...
def collate_fn(batch):
    ...
    list_tensors.append(x)
    m = torch.cuda.FloatTensor(create_molecule_masks(x))
