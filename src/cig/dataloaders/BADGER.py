...
class CigDatasetPreload(CigDatasetBase):
    def __init__(self, conf):
        ...
        # [pathways x (drug + no known)] -> ???
        # self.tg_dti         = sorted(self.dti_dataframe['target_id'].unique().tolist()) # 879 target genes
        # self.tg_dti         = list(map(lambda x: self.idx2gene[x], self.tg_dti))        # 879 target genes  
...
class CigDatasetAutoload(CigDatasetBase):
    ...
    def make_inference_data(self, input_smiles, input_cellid, input_pertid):
        ...
        x = self.create_brics_smiles_set(self.create_rdkit_mol(input_smiles)) 
        x = convert_list_smiles_to_molecules_with_batch_index([x])
        list_tensors.append(x)
    ...
    def make_inference_data_v2(self, input_smiles, input_cellvector, input_pertid):
        list_tensors = []

        x = convert_list_smiles_to_molecules([input_smiles])
        list_tensors.append(x)

        x = torch.cuda.FloatTensor(input_cellvector.reshape(1,-1))
        list_tensors.append(x)
        list_tensors.append([input_smiles])
