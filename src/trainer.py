...
class Trainer:
    def __init__(self, conf, rank, wandb_run=None, ddp_mode=True):
        ...
        self.pathmab_coef  = conf.train_params.chemprof.pathmab_coef
        self.genesab_coef  = conf.train_params.chemprof.genesab_coef
        self.genespo_coef  = conf.train_params.chemprof.gene_spreadout_coef
    ...
    def wandb_lookup_values(self, label, epoch, losses):
        ...
        if len(self.lookup_values['true_dgeR']) > 0:
            ...
            z, zhat = np.where(np.abs(y) < 0, 0, y), np.where(np.abs(yhat) < 0, 0, np.abs(yhat))
            wandb_dict[f'{label}/dge/abs/ndcg']          = ndcg(z, zhat)   

            wandb_dict[f'{label}/dge/rmse']              = rmse(y, yhat)
            wandb_dict[f'{label}/dge/pcorr'], _          = correlation(y, yhat, 'pearson')
            wandb_dict[f'{label}/dge/scorr'], _          = correlation(y, yhat, 'spearman')
    ...
    def get_optimizer(self, model):
        if self.pred_model in ['ciger', 'deepce', 'lesserchemprof', 'chemprof']:
    ...
    def train_valid(self, model, train, train_sampler=None, valid=None):
        ...
        # if not self.debug_mode: model = DDP(model, device_ids=[self.rank], find_unused_parameters=True)
        ...
        for epoch in range(self.num_epochs):
            ...
            if self.rank == 0:
                ...
                if self.best_valid_metric['dge/scorr'] < self.current_valid_metric['dge/scorr']:
                    print("Saving Model Checkpoint with Best Validation Performance... [scorr]")
                    torch.save(model.module.state_dict(), self.checkpoint_path + f'/best_epoch_dge_scorr.mdl')
                if self.best_valid_metric['dge/rmse'] > self.current_valid_metric['dge/rmse']:
                    print("Saving Model Checkpoint with Best Validation Performance... [rmse]")
                    torch.save(model.module.state_dict(), self.checkpoint_path + f'/best_epoch_dge_rmse.mdl')
    ...
    def train_step(self, model, data, epoch=0):
        ...
        for idx, batch in enumerate(data):
            ...
            if self.pred_model in SCHEDULER_BATCHWISE: 
                self.scheduler.step()
            sleep(0.01)
            # I should consider this in ArkDTA project?
            batchwise_loss.append(list(map(lambda x: x.item(), loss)))
            # batchwise_loss.append(loss[0].item())
            #
    ...
    def test(self, model, test):
        ...
        return model
            ...
            # wandb_dict[f'{label}/dge/abs/precision/10']  = precision_k(np.abs(y), np.abs(yhat), 10)
            # wandb_dict[f'{label}/dge/abs/precision/50']  = precision_k(np.abs(y), np.abs(yhat), 50)
            # wandb_dict[f'{label}/dge/abs/precision/100'] = precision_k(np.abs(y), np.abs(yhat), 100)
            # wandb_dict[f'{label}/dge/abs/precision/200'] = precision_k(np.abs(y), np.abs(yhat), 200)
            # wandb_dict[f'{label}/dge/abs/kendall_tau']   = kendall_tau(np.abs(y), np.abs(yhat))
            # wandb_dict[f'{label}/dge/abs/map']           = mean_average_precision(np.abs(y), np.abs(yhat))
            # z, zhat = np.where(np.abs(y) < 0, 0, y), np.where(np.abs(yhat) < 0, 0, np.abs(yhat))
            # wandb_dict[f'{label}/dge/abs/ndcg']          = ndcg(z, zhat)        